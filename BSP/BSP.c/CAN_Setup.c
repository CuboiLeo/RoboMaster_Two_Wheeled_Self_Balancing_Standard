...
void CAN_0x140_Send_Data(CAN_HandleTypeDef *hcanx, uint8_t Motor_ID, uint8_t CMD, uint8_t Data_1, uint8_t Data_2, uint8_t Data_3, 
...
void CAN_SendData(osMessageQId CANx_Handle, CAN_HandleTypeDef *CANx, uint8_t id_type, uint32_t id, uint8_t data[8])
{
    ...
    CAN_Send_Data.CAN_TxHeader.StdId = id;
		CAN_Send_Data.CAN_TxHeader.ExtId = 0x00; 
...
//Specified identifier 0x140 package (for MF9025 V2 Motor)
void CAN_0x140_Send_Data(CAN_HandleTypeDef *hcanx, uint8_t Motor_ID, uint8_t CMD, uint8_t Data_1, uint8_t Data_2, uint8_t Data_3, 
	uint8_t Data_4, uint8_t Data_5, uint8_t Data_6, uint8_t Data_7)
{
	uint8_t data[8];
	...
	CAN_SendData(CAN_SendHandle, hcanx, CAN_ID_STD, (0x140 + Motor_ID), data);
}
